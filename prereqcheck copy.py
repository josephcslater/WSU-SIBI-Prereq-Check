...
prereqdict = {"ME1020":("EGR1010"),
              ...
              "ME4910":(["ME2020","EGR3350","MTH2320","MTH2350","PHY2410","EE2010","ME2210","ME2700","ME3120","ME3310","ME4620c"]),
              "ME7120":(["ME6120"],["ME4120"])}# Verified Aug-16-2016
        ...
        if answer is False:
            if classes_taken[class_name] is 'No grade.':
              ...
              and classes_taken[class_name[:class_name.find('c')]] == '>')  or (class_name[:class_name.find('c')]
                in classes_taken and passed_class(class_name[:class_name.find('c')], classes_taken, course_name)[0]): # corequisite taken earlier and passed?
        answer = True
    else:
...
def pass_all(classes, classes_taken, course_name):
    ...
    for class_name in classes:
        answer, fail_text = passed_class(class_name, classes_taken, course_name)
        if not answer:
            answer = False
            #print('Failed {} with grade of {}'.format(class_name, classes_taken[class_name] ))
            break
...
def satisfied_requirements(requirements, classes_taken, course_name):
    if type(requirements) is str:
        ...
        for requirement in requirements:
            satisfied = satisfied_requirements(requirement, classes_taken, course_name)
...
def check_class(course_name, student_list, data, prereqs, no_transfer_data):
    ...
    for student in student_list:
        ...
        satisfied = satisfied_requirements(prereqs, data["Pre_req_dic"].loc[student], course_name)
...
def check_report(filename, prereqdict, majordict):
    ...
    if course_name in prereqdict:
        prereqs = prereqdict[course_name]
    else:
        print('This will fail ugly because the prerequisites for {} have not been defined in the code.'.format(course_name))
